...
class AdminWindow(QWidget):
    def __init__(
        ...
        # Sync any packets that arrived before AdminWindow connected its signals.
    ...
    # =====================================================
    # 15.1 ADMIN RIGHT PANEL
    # This is the original third admin panel. It remains as
    # the separate right-side admin tools panel.
    # =====================================================
    ...
    def _build_admin_panel(self) -> None:
        ...
        self.admin_users_list.setSpacing(2)
        ...
        self.banned_users_list.setSpacing(0)
        banned_layout.addWidget(self.banned_users_list)

        stats_card = self._make_admin_card("Server Stats")
        stats_card.setFixedHeight(145)

        stats_layout = stats_card.layout()
        stats_layout.setContentsMargins(14, 10, 14, 10)
        stats_layout.setSpacing(3)

        online_row, self.online_count_label = self._make_admin_stat_row(
            "wifi.svg",
            "Online Users: 0"
        )

        room_row, self.room_count_label = self._make_admin_stat_row(
            "door-open.svg",
            "Active Rooms: 0"
        )

        message_row, self.message_count_label = self._make_admin_stat_row(
            "messages-square.svg",
            "Messages Today: 0"
        )

        stats_layout.addWidget(online_row)
        stats_layout.addWidget(room_row)
        stats_layout.addWidget(message_row)

        layout.addWidget(users_card, 4)
        layout.addWidget(banned_card, 2)
        layout.addWidget(stats_card, 0)
    ...
    def _make_admin_stat_row(self, icon_name: str, text: str):
        row = QFrame()
        row.setObjectName("adminStatLabel")
        ...
        text_label.setObjectName("adminStatText")
    ...
    def _make_banned_user_row(self, text: str) -> QWidget:
        row = QWidget()
        ...
        name_label.setObjectName("adminBannedUserText")
    ...
    def _rebuild_admin_users(self) -> None:
        if not hasattr(self, "admin_users_list"):
            return
    ...
    def _make_admin_user_row(self, username: str) -> QWidget:
        ...
        avatar.setObjectName("adminUserAvatar")

        if avatar_path and os.path.exists(avatar_path):
            avatar.setPixmap(self._make_round_pixmap(avatar_path, 34))
        else:
            avatar.setText(display_name[:1].upper())

        name = QLabel(f"{display_name} (You)" if username == self.username else display_name)
    ...
    def _on_admin_data_received(self, packet: dict) -> None:
        ...
        if hasattr(self, "banned_users_list"):
            ...
            if not self.banned_users:
                ...
                for user in self.banned_users:
                    ...
                    item = QListWidgetItem()
                    row = self._make_banned_user_row(text)

                    item.setSizeHint(QSize(0, 34))
                    self.banned_users_list.addItem(item)
                    self.banned_users_list.setItemWidget(item, row)
    ...
    def _moderation_scope_payload(self) -> dict:
        room_name = self.target_name if self.target_mode == "room" else None
        return {
            "target_mode": self.target_mode,
            "room": room_name,
            "chat_title": self.chat_title.text() if hasattr(self, "chat_title") else "",
        }

    def _local_admin_action_notice(self, action: str, username: str) -> None:
        action_word = "kicked" if action == "kick" else "banned"

        if self.target_mode == "room" and self.target_name:
            notice = f"Admin {action_word} {username} from {self.target_name}."
            conversation_key = self._current_conversation_key()
            members = self.room_members.get(self.target_name.lower(), [])
            self.room_members[self.target_name.lower()] = [m for m in members if m.lower() != username.lower()]
            self._apply_target("room", self.target_name)
        elif self.target_mode == "public":
            notice = f"Admin {action_word} {username} from Public Chat."
            conversation_key = "public"
        else:
            notice = f"Admin {action_word} {username}."
            conversation_key = self._current_conversation_key()

        self._add_notice_to_conversation(conversation_key, notice)
    ...
    def _kick_user(self, username: str) -> None:
        ...
        scope = self._moderation_scope_payload()
        try:
            self.network_client.kick_user(
                username,
                target_mode=scope.get("target_mode"),
                room=scope.get("room"),
            )
        except TypeError:
    ...
    # =====================================================
    # 18. ADMIN VISIBILITY
    # Header admin buttons were intentionally removed.
    # The right-side admin panel remains the only place for actions.
    # =====================================================
    ...
    # =====================================================
    # 31. ADMIN ACTIONS
    # Header admin buttons were removed. These wrappers are kept only
    # for backwards compatibility if any old signal still references them.
    # =====================================================

    def _kick_selected_target(self) -> None:
        if self.target_mode == 'private' and self.target_name:
            self._kick_user(self.target_name)

    def _ban_selected_target(self) -> None:
        if self.target_mode == 'private' and self.target_name:
            self._ban_user(self.target_name)
